...
class HomeExecutor
    ...
    constructor(
        ...
        suspend fun execute(login: String): OverViewTabData {
            ...
            val user =
                userResponse.data?.user ?: throw IllegalStateException(
            ...
            client
                .query(GetUserPinnedItemsQuery(login))
                .execute()
                .data
                ?.user
                ?.pinnedItems
                ?.let { pinnedItems ->
                    if (pinnedItems.nodes.isNullOrEmpty()) {
                        ...
                        popularRepos =
                            client
                                .query(GetUserPopularReposQuery(login, first = 5))
                                .execute()
                                .data
                                ?.user
                                ?.repositories
        ...
        suspend fun followUser(userId: String): FollowUserMutation.Data =
            client.mutation(FollowUserMutation(user = userId)).execute().dataAssertNoErrors

        suspend fun unFollowUser(userId: String): UnfollowUserMutation.Data =
            client.mutation(UnfollowUserMutation(user = userId)).execute().dataAssertNoErrors
    }
